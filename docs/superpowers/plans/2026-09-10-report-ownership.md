...
**Files:** modify `internal/service/service.go`, `internal/tracker/tracker.go`; create `internal/service/reporting.go`, `docs/REPORT-OWNERSHIP.md`.
...
- [ ] Run gofmt, git diff --check, offline `go build ./internal/service ./internal/tracker`, `go vet ./internal/service ./internal/tracker`, and `go build ./...` with Go1.26.3 and the specified offline environment. Do not run tests or install anything. Report failures honestly.
- [ ] Commit only the four scoped files. Write the requested report with base/head SHAs, exact commands/status/output, self-review of report admission/close orderings and snapshot aliasing, and limitations. No push; controller handles independent review and draft PR.
